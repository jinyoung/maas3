<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['statisticId','assistanceType','numberOfRequests','averageResponseTime',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="statisticId" header="statisticId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="assistanceType" header="assistanceType" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="numberOfRequests" header="numberOfRequests" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="averageResponseTime" header="averageResponseTime" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">AssistanceStatistics 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <AssistanceStatistics :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="assistanceStatisticsModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
<script>
import AssistanceStatistics from '../components/AssistanceStatistics.vue'
...
export default {
    name : 'assistanceStatistics-grid',
    mixins:[AssistanceStatistics],
    components:{
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.assistanceStatistics.href;
