<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['passengerName','currentLocation','locationLastUpdate',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="passengerName" header="passengerName" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="currentLocation" header="currentLocation" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="locationLastUpdate" header="locationLastUpdate" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">PassengerLocation 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <PassengerLocation :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="passengerLocationModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
<script>
import PassengerLocation from '../components/PassengerLocation.vue'
...
export default {
    name : 'passengerLocation-grid',
    mixins:[PassengerLocation],
    components:{
        PassengerLocation,
    },
    data: () => ({
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.passengerLocation.href;
