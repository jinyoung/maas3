<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['notificationId','driverName','notificationType',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="notificationId" header="notificationId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="driverName" header="driverName" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="notificationType" header="notificationType" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">DriverNotification 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <DriverNotification :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="driverNotificationModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
export default {
    name : 'driverNotification-grid',
    mixins:[DriverNotification],
    components:{
        DriverNotification,
    },
    data: () => ({
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.driverNotification.href;
