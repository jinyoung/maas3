<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['requestId','passengerName','requesterName','requestStatus',]" />
            <wj-flex-grid-column binding="index" header="Number" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="requestId" header="requestId" width="2*" :isReadOnly="true" align="center" />
            ...
            <wj-flex-grid-column binding="requestStatus" header="requestStatus" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
            ...
            >
                ...
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">AssistanceRequest 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
                    ...
                    <v-card-text>
                    <div>
                        <AssistanceRequest :offline="offline"
                            :isNew="!selectedItem"
                            :editMode="true"
                            v-model="assistanceRequestModel"
                            @add="append"
                        />
                    </div>
                    </v-card-text>
...
export default {
    name : 'assistanceRequest-grid',
    mixins:[AssistanceRequest],
    components:{
        AssistanceRequest,
    },
    data: () => ({
        ...
        async deleteSelectedRows() {
            try {
                if (!this.offline) {
                    ...
                    if (view.currentItem) {
                        const deleteUrl = view.currentItem._links.assistanceRequest.href;
